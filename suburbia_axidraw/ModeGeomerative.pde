class ModeGeomerative extends ModeVoronoi
{
  ...
  void initControls()
  {
    ...
    cp5.addSlider(cp5_prefix+"_segmentLength").setGroup(this.cp5Group).setHeight(cp5_height).addListener(this).setPosition(5, 25).setRange(10, 100).setValue(segmentLength);
    ;
    cp5.end();
  ...
  void distributePoints()
  {
    ...
    shp = RG.centerIn(shp, applet.g, 20);
  ...
  public void resize()
  {
    println("ModeGeomerative.resize(), width="+width+", height="+height);
    distributePoints();
    this.areas = null;
    generate();
  }
